...
double metodoNewton(double d, EqCorda &p, double eps, int itMax)
{
    double fx = p.f(d);
    if (abs(fx) < eps)
        return d;
...
// NEWTON ORIGINAL ITERATIVO COM DERIVADA NUMERICA

double metodoNewtonDdxNum(double d, EqCorda &p, double eps, int itMax)
{
    double fx = p.f(d);
    if (abs(fx) < eps)
        return d;
...
double metodoNewtonRec(double d, EqCorda &p, double eps, int k, int itMax)
{
    double fx = p.f(d);
    if (abs(fx) < eps)
        return d;
    ...
    else if (k >= itMax)
    {
        ...
        return metodoNewtonRec(dk, p, eps, k + 1, itMax);
}
